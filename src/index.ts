...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (toolName === "iniciar_planejamento_projeto") {
    const objetivo = args.objetivo_principal as string;
    const tecnologias = (args.tecnologias_desejadas as string[]) || ["A definir"];
  ...
  if (toolName === "gerar_documento_requisitos_ears") {
    const funcionalidades = args.funcionalidades as Array<{nome: string, condicao_previa: string, evento_gatilho: string, resposta_esperada: string}>;
  ...
  if (toolName === "salvar_documento") {
    const caminho_arquivo = args.caminho_arquivo as string;
  ...
  if (toolName === "gerar_estrutura_pastas") {
    const pastas = args.pastas as string[];
  ...
  if (toolName === "criar_plano_de_tarefas") {
    const tarefas = args.tarefas as string[];
    const caminho_arquivo = args.caminho_arquivo as string;
  ...
  if (toolName === "gerar_diagrama_mermaid") {
    const caminho_arquivo = args.caminho_arquivo as string;
    const codigo_mermaid = args.codigo_mermaid as string;
  ...
  if (toolName === "gerar_pipeline_github_actions") {
    ...
    const passos = (args.passos as string[]) || ["install", "test"];
    ...
    if (stack.toLowerCase() === "node" || stack.toLowerCase() === "react") {
      ...
      if (passos.includes("install")) actionsCode += `      - run: npm install\n`;
      if (passos.includes("lint")) actionsCode += `      - run: npm run lint\n`;
      if (passos.includes("test")) actionsCode += `      - run: npm test\n`;
      if (passos.includes("build")) actionsCode += `      - run: npm run build\n`;
    } else {
  ...
  if (toolName === "atualizar_contexto_da_arquitetura") {
    const novaRegra = args.nova_regra as string;
    const contextPath = path.resolve(process.cwd(), "AI_CONTEXT.md");
  ...
  if (toolName === "gerar_esqueleto_de_testes") {
    ...
    const fw = (args.framework as string).toLowerCase();
    const cenarios = args.cenarios as string[];
  ...
  if (toolName === "commit_small_release") {
    const msg = args.mensagem_commit as string;
    const projectPath = process.cwd();
